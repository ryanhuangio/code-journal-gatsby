...
const BaseTemplate = ({ children, pageTitle }) => {
  ...
  const [isDarkIcon, setIsDarkIcon] = useState(null);
  useEffect(() => {
    if (
  ...
  return (
    <div className="max-w-screen max-h-full dark:bg-slate-900 dark:text-gray-300 p-6">
      ...
      <div className="mx-auto align-middle text-left max-w-3xl mb-6">
        <button
          onClick={toggleTheme}
          className="absolute right-5 top-3 night-mode-toggle rounded-full border w-12 h-12 opacity-60 hover:opacity-100 transition-all dark:border-teal-700 dark:hover:border-teal-500 border-gray-500 hover:border-gray-600"
        >
          {isDarkIcon ? (
            <FontAwesomeIcon
              icon={faSun}
